---
layout: post
title: 'Vibe coding the vibe coder: using LangChain4j to build an agentic coder'
date: 2026-03-30T00:00:00Z
tags: ai llm agents
synopsis: 'Vibe coding the vibe coder: using LangChain4j to build an agentic coder'
author: mariofusco
---
:imagesdir: /assets/images/posts/agentic

It's 2026, and handcrafting code seems to suddenly have become a thing of the past. 
Software engineering has adopted code assistants en-masse with development now being more like conducting and orchestrating coding agents that write the code, run tests, and fix bugs on their behalf.

With this new trend in mind, we thought it would be a good exercise to put the LangChain4j agentic framework to work to build an agentic coder. Even more interesting, we went a bit meta and let a code assistant design and implement the LangChain4j-based agentic coder. This article discusses the outcomes of this experiment, which produced the agentic coder now available in https://github.com/mariofusco/langchain4j-agentic-coder[this project].
...
and a test that clones the folder containing that `Calculator` class into a temporary directory, then executes the LangChain4j agentic coder on it.
...
After this first experiment, we asked the code assistant to redesign the agentic coder using a workflow-based approach instead of a supervisor-based one. It came up with the following architecture: a sequence of five steps similar to the four previous ones, plus a fifth step that prints a summary of all actions taken during execution—something the supervisor handles autonomously without a dedicated agent.
...
The other major difference from the supervisor-based implementation is that this time, both the planning and execution steps aren't single agents but are implemented with a loop structure that allows these steps to perform multiple iterations and adjust and improve their work before moving forward.
...
We've also seen how to put that system to work on a real debugging session and how to monitor its execution and visualize its topology. The results were impressive—the system fixed all the bugs in the code and made all the tests pass, with an articulated topology of agents working together to accomplish the task. We've also seen how a workflow-based approach can be more efficient than a supervisor-based one, even when it involves more agents, because of the lower overhead.